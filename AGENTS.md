...
Lorume is currently in product definition and early engineering setup. The repository is becoming the control plane for operating an Agent Network. It now has a Runtime Fleet page with read-only Runtime Skill deep links, Agent ownership filtering, and Agent 看板 deep links, a read-only Skill 仓库 page over already-collected Runtime Skill metadata, a read-only Runs conversation task page, a read-only 定时任务 page over collected Runtime schedule definitions and execution history, an Agent 看板 page over OpenClaw `main` Agent analysis reports, top-right Operations and Notifications utility drawers, organization settings for invitations, collector-backed runtime collection using the current `Device / Runtime / Agent / Task` model, organization-based auth/access, a shadcn/ui frontend design-system foundation, and a standalone backend with Postgres-backed query APIs, production-like Docker / Nginx deployment files, an SSH-operated production deployment at `lorume.com`, plus an outbound WebSocket device control channel for connection health. It does not yet have centralized Skill management, Agent migration, multi-device orchestration, or runtime execution control.
...
- `docs/product/runtime-scheduled-task-page-spec.md`: TinySpec for the dedicated scheduled Task page, Runtime schedule probe snapshots, OpenClaw cron mapping, scheduled execution grouping, and harness boundaries.
...
- `docs/product/collector-upgrade-spec.md`: TinySpec for server-controlled collector package manifest, restricted WebSocket upgrade messages, collector self-upgrade, and Operation / Job progress tracking.
- `docs/product/cli-device-capability-spec.md`: current rules for the deterministic local `lorume` CLI device capability atoms.
- `docs/product/agent-skill-probing-spec.md`: current rules for read-only target-local Runtime Skill probing metadata, Agent ownership mapping, statuses, APIs, and UI boundaries.
...
- `docs/product/openclaw-agent-dashboard-backend-spec.md`: TinySpec for backend-only OpenClaw `main` Agent analysis Operations, WS prompt dispatch, collector execution, report persistence, scheduler, and harness.
- `docs/product/openclaw-agent-dashboard-frontend-spec.md`: TinySpec for the OpenClaw `main` Agent 看板 frontend page, API adapter, report display, Operation progress, unsupported target boundaries, and harness.
...
- `src/runtime/runtime-scheduled-task-query.ts`: frontend API adapter and filter helper for backend scheduled Task group and execution-history query responses.
...
- `src/runtime/runtime-skill-inventory.ts`: frontend source of truth for read-only Skill 仓库 aggregation, Runtime/Agent filtering, derived available Agents, and summary counts.
- `src/runtime/SkillWarehousePage.tsx`: protected read-only Skill 仓库 page for browsing already-collected Runtime and Agent Skill metadata.
- `src/runtime/RuntimeScheduledTasksPage.tsx`: protected read-only 定时任务 page for browsing collected schedule definitions, next-run metadata, recent status, and grouped execution history.
...
- `src/agent-dashboard/agent-dashboard-query.ts`: frontend API adapter for Agent Analysis report, run, and Operation detail responses.
- `src/agent-dashboard/AgentDashboardPage.tsx`: protected Agent 看板 page for OpenClaw `main` Agent hard metrics, Agent self-analysis, report history, and analysis Operation status.
...
- `src/server/runtime-http-api.ts`: backend HTTP API for collector ingestion, Runtime Fleet / Runs query endpoints, Runtime / Agent Skill probe snapshot endpoints, and ingestion diagnostics.
...
- `scripts/check-device-package-version.mjs`: repo harness guard that requires collector / CLI device package code changes to bump the device package version and keeps package, collector, and adapter version sources aligned.
...
- Only expose implemented, user-verifiable capabilities in navigation, homepage CTAs, and page-level action buttons. Current Console navigation is `Runtime Fleet`, `Runs`, `定时任务`, `Skill 仓库`, `Agent 看板`, and `组织设置`; Operations and Notifications are top-right utility drawers reachable through `/operations` and `/notifications`, not primary nav pages. Future surfaces such as Agent Studio, Workflow Studio, Object Catalog, centralized Skill management, People, Integrations, and Governance stay in docs/backlog until their page, data path, permissions, and harness exist.
- Keep URL routes durable and minimal: `/` is the public homepage, `/login` is the auth entry, `/invite/:token` is the invitation entry, and `/runtime`, `/runs`, `/scheduled-tasks`, `/skills`, `/agent-dashboard`, `/settings` are the current protected Console pages. `/operations` and `/notifications` are protected utility drawer routes layered over the current Console context. Unknown or not-yet-built routes should fall back to the current default Console page instead of exposing fake data.
...
- Runs conversation task pages must stay task-context first: do not render unlinked runtime executions, listening status, capability gaps, adapter evidence, raw limitations, command names, or debugging notes as user-facing task cards. Current `/runs` always queries `taskType=conversation`; scheduled Task definitions and grouped execution history live on `/scheduled-tasks` and follow `docs/product/runtime-scheduled-task-page-spec.md`. If a platform cannot provide creator, assignee, group/channel, message excerpt, or execution state for a real work item, show a concise unsupported/unknown/user-facing fallback and keep details in logs/spec/harness. Do not display raw DingTalk `cid...`, phone numbers, open conversation ids, or other opaque external ids as conversation names. For DingTalk direct chats without a readable person name, show `DingTalk 私聊`; for groups without a readable name, show `DingTalk 群聊`. A real work item with no linked execution should say `未关联执行`, not `不支持采集`.
...
| Runtime Skill probing and Skill 仓库 | `docs/product/agent-skill-probing-spec.md`, `src/runtime/runtime-skill-probe.ts`, `src/runtime/agent-skill-probe.ts`, `src/runtime/runtime-skill-inventory.ts`, `src/runtime/SkillWarehousePage.tsx`, `src/server/runtime-http-api.ts`, `src/runtime/RuntimeFleetPage.tsx` | `src/runtime/runtime-skill-probe.test.ts`, `src/runtime/runtime-skill-inventory.test.ts`, `src/runtime/agent-skill-probe.test.ts`, `src/server/runtime-http-api-skill-probe.test.ts`, `src/runtime/RuntimeFleetPage.skill-probe.test.tsx`, `src/App.test.tsx`, `e2e/runtime-fleet.spec.ts`, `npm run check:runtime`, `npm run check:backend`, `npm run check:quick`, `npm run check:e2e` |
| Runtime scheduled Task probe, grouping, and page | `docs/product/runtime-scheduled-task-page-spec.md`, `src/runtime/runtime-schedule-probe.ts`, `src/runtime/runtime-scheduled-task-query.ts`, `src/runtime/RuntimeScheduledTasksPage.tsx`, `src/runtime/runtime-task-sync.ts`, `scripts/lorume-runtime-adapters.mjs`, `scripts/lorume-device-collector.mjs`, `src/server/postgres-store.ts`, `src/server/runtime-http-api.ts`, `src/App.tsx` | `src/runtime/runtime-schedule-probe.test.ts`, `src/runtime/runtime-scheduled-task-query.test.ts`, `src/App.test.tsx`, `src/cli/lorume-cli.test.ts`, `src/runtime/device-collector-script.test.ts`, `src/server/postgres-store.test.ts`, `src/server/runtime-http-api-postgres.test.ts`, `npm run check:runtime`, `npm run check:cli`, `npm run check:backend`, `npm run check:db`, `npm run check:quick` |
...
| Collector package upgrade | `docs/product/collector-upgrade-spec.md`, `docs/product/runtime-device-registration-spec.md`, `docs/product/backend-service-spec.md`, `docs/product/operation-job-runner-spec.md`, `docs/product/runtime-fleet-page-spec.md`, `src/server/runtime-control-channel.ts`, `src/operations/operation-store.ts`, `src/operations/job-runner.ts`, `src/backend/device-installer-http-api.ts`, `scripts/lorume-device-collector.mjs`, `scripts/install-device-collector.sh`, `scripts/check-device-package-version.mjs`, `src/runtime/RuntimeFleetPage.tsx`, `src/console/ConsoleUtilityDrawer.tsx` | `src/server/runtime-control-channel.test.ts`, `src/operations/operation-store.test.ts`, `src/operations/job-runner.test.ts`, `src/backend/backend-server.test.ts`, `src/runtime/device-collector-script.test.ts`, `scripts/check-device-package-version.test.mjs`, `src/console/ConsoleUtilityDrawer.test.tsx`, `src/App.test.tsx`, `e2e/runtime-backend-api.spec.ts`, `e2e/runtime-fleet.spec.ts`, `npm run check:device-package-version`, `npm run check:backend`, `npm run check:runtime`, `npm run check:db`, `npm run check:quick`, `npm run check:backend:e2e`, `npm run check:e2e` |
| OpenClaw Agent Dashboard backend | `docs/product/openclaw-agent-dashboard-backend-spec.md`, `docs/product/backend-service-spec.md`, `docs/product/runtime-device-registration-spec.md`, `docs/product/operation-job-runner-spec.md`, `db/schema.sql`, `src/agent-analysis/`, `src/operations/agent-analysis.ts`, `src/server/runtime-control-channel.ts`, `src/backend/backend-server.ts`, `scripts/lorume-device-collector.mjs` | `src/agent-analysis/agent-analysis-model.test.ts`, `src/agent-analysis/agent-analysis-http-api.test.ts`, `src/agent-analysis/agent-analysis-store.test.ts`, `src/operations/agent-analysis.test.ts`, `src/server/runtime-control-channel.test.ts`, `src/runtime/device-collector-script.test.ts`, `npm run check:backend`, `npm run check:runtime`, `npm run check:db`, `npm run check:device-package-version` |
| OpenClaw Agent Dashboard frontend | `docs/product/openclaw-agent-dashboard-frontend-spec.md`, `docs/product/ui-design.md`, `docs/product/design/surface-register.md`, `docs/product/design/page-patterns.md`, `src/agent-dashboard/`, `src/App.tsx`, `src/components/layout/AppShell.tsx`, `src/runtime/RuntimeFleetPage.tsx`, `src/HomePage.tsx` | `src/agent-dashboard/agent-dashboard-query.test.ts`, `src/agent-dashboard/AgentDashboardPage.test.tsx`, `src/App.test.tsx`, `e2e/agent-dashboard.spec.ts`, `npm run check:quick`, `npm run check:e2e`, `npm run check:repo` |
...
Equivalent package entry points are `npm run verify`, `npm run check`, and `npm run harness`. If the local agent runtime has Node but no `npm` binary, `./scripts/verify.sh` falls back to `scripts/run-package-script.mjs` so the same harness still runs. The full harness verifies required product documents/assets, local Markdown links, device package version bump rules, Postgres schema checks, backend store/control/API checks, backend API-only E2E, backend bundle and deploy config checks, collector script behavior, TypeScript typecheck, unit/component tests, production build, and the Playwright responsive layout harness.
...
| `npm run check:device-package-version` | Verify collector / CLI device package changes include a higher package version and aligned version sources. | Collector script, runtime adapter, local CLI, installer, or device package manifest changes. |
