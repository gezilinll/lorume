...
| CLI device capability atoms | `docs/product/cli-device-capability-spec.md`, `scripts/lorume.mjs` | `src/cli/lorume-cli.test.ts`, `npm run check:cli`, `npm run check:runtime`, `npm run check:backend`, `npm run check:quick` |
...
| `npm run check:backend` | Focused local backend store, control channel, HTTP API, Agent Skill probe API, and collector POST / WebSocket harness. | Runtime snapshot API, backend API handler, Agent Skill probe request/snapshot paths, collector posting, device WebSocket, inventory + work-state refresh command lifecycle, or backend persistence changes. |
