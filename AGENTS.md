...
| Collector package upgrade | `docs/product/collector-upgrade-spec.md`, `docs/product/runtime-device-registration-spec.md`, `docs/product/backend-service-spec.md`, `docs/product/operation-job-runner-spec.md`, `docs/product/runtime-fleet-page-spec.md`, `src/server/runtime-control-channel.ts`, `src/operations/operation-store.ts`, `src/operations/job-runner.ts`, `src/backend/device-installer-http-api.ts`, `scripts/lorume-device-collector.mjs`, `scripts/install-device-collector.sh`, `src/runtime/RuntimeFleetPage.tsx`, `src/console/ConsoleUtilityDrawer.tsx` | `src/server/runtime-control-channel.test.ts`, `src/operations/operation-store.test.ts`, `src/operations/job-runner.test.ts`, `src/backend/backend-server.test.ts`, `src/runtime/device-collector-script.test.ts`, `src/console/ConsoleUtilityDrawer.test.tsx`, `src/App.test.tsx`, `e2e/runtime-backend-api.spec.ts`, `e2e/runtime-fleet.spec.ts`, `npm run check:backend`, `npm run check:runtime`, `npm run check:db`, `npm run check:quick`, `npm run check:backend:e2e`, `npm run check:e2e` |
