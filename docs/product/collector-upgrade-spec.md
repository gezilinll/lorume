...
- Device 卡片展示 `Collector <version>` 以及 `最新`、`待升级`、`升级中`、`升级失败`、`需手动重装` 或 `未上报` 版本状态。
- Device 详情面板展示当前版本、最新版本、升级能力、最近升级 Operation、最近升级时间和失败摘要。
- 详情面板提供 `升级 Collector` / `重试升级` / `查看任务` 操作。
...
- Runtime Fleet component harness：展示最新版本、每台设备版本状态、单设备升级按钮和 Operation deep link。
