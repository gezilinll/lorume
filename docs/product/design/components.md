...
- Skill full detail dialogs are reading surfaces, not metadata summaries. Their header contains only the Skill title, and the body contains `本地路径` plus `Skill 正文`. The body text area should expand to use the remaining vertical space. Copy actions for paths and bodies are icon-only controls that appear on content hover/focus in the content area's top-right corner; do not use persistent text buttons inside these code-like reading blocks.
