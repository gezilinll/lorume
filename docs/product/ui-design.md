...
- 左侧栏顶部显示当前组织，作为未来组织切换和组织上下文动作的入口。
- 左侧栏中部只放当前已实现主导航：Runtime Fleet、Runs / Work Board、组织设置。
- 左侧栏底部显示个人头像入口；邮箱、退出登录和后续个人偏好放在个人菜单里，不在 rail 上常驻展示。
- 个人菜单不重复当前组织和当前角色，避免把已经由组织入口表达的信息再展示一遍。
