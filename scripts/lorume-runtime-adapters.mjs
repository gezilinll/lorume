...
async function collectSlockWorkState(deviceId, observedAt, config) {
  ...
  const fallbackChannels = configuredChannels.map(normalizeSlockChannel);
  ...
  const hasTaskBoardEvidence = workItems.length > 0 || warnings.length < fallbackChannels.length;
  ...
  return {
    ...
    capabilities: [
      slockWorkStateCapability(observedAt, {
        workItemsSupport: hasTaskBoardEvidence ? "supported" : "unknown",
        workItemsEvidence: hasTaskBoardEvidence
...
export function collectInventorySnapshot(config = {}, args = {}) {
  const mergedConfig = {
    ...config,
...
export async function collectWorkStateSnapshot(config = {}, args = {}) {
  const mergedConfig = {
    ...config,
    ...(args.deviceId ? { deviceId: args.deviceId } : {}),
  };
