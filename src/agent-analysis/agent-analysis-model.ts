...
export function buildAgentAnalysisPrompt(input: {
  ...
  return [
    ...
    [
      ...
      "7. 不要穷尽扫描所有历史；优先使用索引、摘要、任务列表或最近记录定位周期内会话，再选择最能代表任务类型和反馈信号的会话深读。",
      "8. 深度分析最多覆盖 12 个代表性会话；如果周期内会话更多，请用系统统计辅助估算任务量，并在案例和证据中保留最有代表性的记录。",
      "9. 不要修改文件、发送消息、执行外部任务或触发任何会产生副作用的操作。只允许读取和分析历史记录。",
      "10. 如果某些会话证据不足，可以跳过，不要编造。",
    ].join("\n"),
    [
      "分析步骤：",
      "1. 找出周期内相关会话和记录片段。",
    ...
    "输出要求：只输出 JSON；不要 markdown；不要输出分析过程；输出 JSON 必须符合 outputJsonContract，不要增加其他字段；taskTypes 最多 6 类，每类 cases 最多 3 个，risks 最多 5 条，actions 最多 5 条；evidenceIds 和 cases[].id 使用会话 id、trajectory id、task id 或你能稳定引用的历史记录 id。",
